...
def _windows_launcher() -> str:
    return "\r\n".join([
        ...
        "  echo Despues vuelva a ejecutar INICIAR_APP_WINDOWS.bat.",
        "  echo.",
        "  pause",
        "  exit /b 1",
        ")",
        "echo Iniciando EIA-Agent Cliente...",
        "echo Mantenga esta ventana abierta mientras utiliza la app.",
        "start \"\" powershell.exe -NoProfile -WindowStyle Hidden -Command \"Start-Sleep -Seconds 2; Start-Process 'http://127.0.0.1:8765/'\"",
        "echo App disponible en http://127.0.0.1:8765/",
        "\"%PYTHON_EXE%\" %PYTHON_ARGS% \"server\\eia_client_server.py\"",
        "echo.",
