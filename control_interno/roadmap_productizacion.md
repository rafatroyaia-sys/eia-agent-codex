...
- ~~**QA-BE03**: Prueba real de inicialización de expediente limpio~~ — **COMPLETADO ✅ 2026-05-29** (5/5 pruebas OK: normal+idempotencia+no-borrado+no-guides+force, 20 carpetas+6 archivos verificados en ejecución real, 0 bugs detectados, suite 6600 OK, `control_interno/qa_be03_prueba_inicializacion_expediente.md`)
- ~~**BE-04**: Gestión segura de AEMET API key~~ — **COMPLETADO ✅ 2026-05-29** (99 tests OK, `src/eia_agent/core/config_manager.py`, `mask_secret`+`is_placeholder_value`+`validate_config`+`scan_repo_for_potential_secrets`+`ConfigValidationResult`, CLI `config-check [--write]` + `secrets-scan [--write]`, 7 códigos BE04-E/W/I, sin llamadas externas/IA/web, .gitignore actualizado, suite 6699 OK, `docs/CONFIG_MANAGER.md`)
- ~~**QA-BE04**: Prueba real de configuración segura y escaneo de secretos~~ — **COMPLETADO ✅ 2026-05-29** (8/8 pruebas OK: config-check limpio+.env sintético+secrets-scan detección+enmascaramiento+exclusiones tmp/venv/.git+.gitignore, 0 claves reales expuestas, 2 incidencias informativas sin corrección, suite 6699 OK, `control_interno/qa_be04_configuracion_segura.md`)
- ~~**BE-04.1**: Excluir `.claude/` del escaneo de secretos por defecto~~ — **COMPLETADO ✅ 2026-05-30** (`.claude` añadido a `_DEFAULT_EXCLUDE_DIRS` en `config_manager.py`, 6 tests nuevos `TestExcludeDotClaude`, `docs/CONFIG_MANAGER.md` actualizado, incidencia QA-BE04 resuelta, suite 6705 OK)
...
- ~~**EN-04**: Eliminar referencia huerfana numbering.xml~~ — **COMPLETADO ✅ 2026-05-30** (127 tests OK, `src/eia_agent/core/document_numbering_manager.py`, `validate_docx_file`+`docx_has_numbering_definitions`+`is_numbered_list_candidate`+`is_bullet_list_candidate`+`select_numbered/bullet_style`+`analyze_docx_numbering`+`apply_list_styles_to_docx`+`process_document_numbering`, 4 codigos EN04-E/W, CLI `document-numbering [--write] [--apply]`, sin IA/web/modificar original/reordenar parrafos, OBS-004 resuelta via generacion controlada numbering.xml en copia, suite 6932 OK, `docs/DOCUMENT_NUMBERING_MANAGER.md`)
...
- ~~**PIPE-05**: Integrar RD-07 impactos positivos con gaps ALTA en pipeline técnico y AU-04~~ — **COMPLETADO ✅ 2026-05-29** (pipeline 18→19 pasos: añade AUDIT_POSITIVE_GAPS en pos. 10 entre AUDIT_CONDITIONAL_CHAINS y PHASE6_CUMULATIVE; AU-04 acepta `positive_gap_data`, None → sin issue; 10 tests `TestNewAuditStepsPIPE05` + 20 tests `TestExtractFinalIssuesFromPositiveGaps` en AU-04; markdown AU-04 pasa de 14→15 secciones; suite 6506 OK)
...
- ~~**QA-10**: Prueba real del pipeline completo con IM-09 integrado~~ — **COMPLETADO ✅ 2026-05-29** (18/18 pasos OK sobre copia NAVE-222, AUDIT_CONDITIONAL_CHAINS pos. 9, conditional_chain_result.json+md generados, final_audit_result.json incorpora conditional_chain_summary is_valid:true, sección 9 IM-09 en markdown audit-final; 1 bug menor corregido: is_valid derivado de error_count en _build_summary_from_conditional_chains; test regresión añadido; suite 6341 OK, `control_interno/qa10_pipeline_im09_nave222.md`)
- ~~**DOC-09**: Incorporacion de IM-09 en documento final, QC, paquete y checklist~~ — **COMPLETADO ✅ 2026-05-29** (Bloque C.5 resumen auditoria IM-09; Bloque I.3 aviso IM-09 (Sintesis → I.4); QC: `check_conditional_chain_visibility` + QC-E009/W009/W010; package: `conditional_chain_result.json/.md` en 02_auditorias; preparer: campo `conditional_chain_status` + CHK-013 (13 items); +37 tests en 4 archivos; suite 6376 OK; `docs/DOCUMENT_MARKDOWN_BUILDER.md`, `DOCUMENT_QUALITY_CHECKER.md`, `DOCUMENT_PACKAGE_BUILDER.md`, `DOCUMENT_PRESENTATION_PREPARER.md` actualizados; commit `DOC-09: incorporar IM-09 en documento y paquete final`)
- ~~**QA-11**: Prueba real del pipeline completo con RD-07 integrado~~ — **COMPLETADO ✅ 2026-05-29** (19/19 pasos OK sobre copia NAVE-222 `tmp/qa11_pipeline_rd07_nave222_20260529_115852`, AUDIT_POSITIVE_GAPS pos. 10 SUCCESS, `positive_gap_result.json`: 2 impactos revisados, 0 positivos, 0 con gap ALTA, 0 errores; `positive_gap_summary` en `final_audit_result.json` (available:True, status:OK), nota `Estado RD-07: disponible.`; sección `## 9. Resultado RD-07` en `final_audit_result.md` (15 secciones totales); 0 bugs de código; suite 6506 OK, `control_interno/qa11_pipeline_rd07_nave222.md`)
- **IM-06** código: template C.5 en IM-01
- ~~**RD-07** código: validador impactos positivos con gap ALTA y nota de incertidumbre~~ — **COMPLETADO ✅ 2026-05-29** (99 tests OK, `src/eia_agent/core/positive_impact_gap_validator.py`, RD07-E001/E002/W001/W002, negación contextual, `administrative_ready=False`, CLI `audit-positive-gaps [--write]`, suite 6475 OK, `docs/POSITIVE_IMPACT_GAP_VALIDATOR.md`)
...
- ~~**RELEASE-01**: Prueba completa de producto v1.0 desde expediente limpio hasta paquete final~~ — **COMPLETADO ✅ 2026-05-31** (15 comandos ejecutados, 0 FAILED/BLOCKED en pipeline 19/19 pasos, 19/19 outputs documentales generados, document-qc VÁLIDO 11/11 bloques 6 figuras, ZIP 3.2 MB 23 archivos, DOCX con TOC 349 párrafos, administrative_ready=False, IM-09+RD-07 presentes, 5 incidencias menores esperadas, 0 correcciones de código, suite final 7074 OK, `control_interno/release01_prueba_producto_v1.md`)
- ~~**CI-01**: GitHub Actions — workflow de validación automática~~ — **COMPLETADO ✅ 2026-05-31** (`.github/workflows/ci.yml`: 9 pasos — checkout + Python 3.11 + pip + requirements + imports + unittest + archivos prohibidos + secrets-scan informativo; windows-latest; triggers push/PR master + workflow_dispatch; secrets-scan continue-on-error=true por falsos positivos conocidos QA-BE04; suite local 7074 OK; `docs/CI_GITHUB_ACTIONS.md`; README.md actualizado)
