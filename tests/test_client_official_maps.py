...
from eia_agent.core.client_official_maps import (
    ...
    build_noise_roads_lden_wms_url,
...
class TestClientOfficialMaps(unittest.TestCase):
    ...
    def test_build_noise_roads_lden_wms_url_contains_required_parameters(self):
        url = build_noise_roads_lden_wms_url(28.963, -13.551)

        self.assertIn("SERVICE=WMS", url)
        self.assertIn("REQUEST=GetMap", url)
        self.assertIn("SRS=EPSG%3A4326", url)
        self.assertIn("LAYERS=AM.NoiseRestrictionZone", url)
    ...
    def test_generate_client_official_maps_writes_png_with_fetcher(self):
        ...
        noise = self.tmp / "cartografia" / "mapas" / "MAP-OFICIAL-006_ruido_carreteras_lden.png"
        ...
        self.assertEqual(noise.read_bytes(), png)
        self.assertEqual(len(result["maps"]), 6)
    ...
    def test_generate_client_official_maps_skips_without_coordinates(self):
        ...
        self.assertFalse((self.tmp / "cartografia" / "mapas" / "MAP-OFICIAL-006_ruido_carreteras_lden.png").exists())
